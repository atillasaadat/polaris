...
---------------------------------------
It is the *sizing* half of a report page: which sections exist, in what order,
what goes in each table, and the one sentence at the top. The other half — the
stylesheet, the behaviour script, the KaTeX and figure plumbing, the prose
helpers and the page shell — is :mod:`analysis.common.report_html`, shared with
every other report. The split is by ownership, not by convenience: the design
system is nobody's analysis, and the sections are nothing but.
...
(:func:`analysis.common.report_html.glossary`), said once per family rather than repeated on every row. It
...
the way in. Formulae go through :mod:`analysis.common.texmath`, which typesets
the LaTeX **the report objects carry themselves** with KaTeX; everything else
goes through :mod:`analysis.common.mathfmt`, which sets ``N.m.s`` as ``N·m·s``
...
Self-containment (nothing fetched at runtime) and escaping (every value through
:func:`html.escape` before it reaches the page) are properties of the shell, not
of this module; :mod:`analysis.common.report_html` states and keeps them.
...
from analysis.common.texmath import tex_html, tex_symbol, tex_value
from analysis.common.report_html import (
    esc as _esc,
    figure_block as _figure_block,
    glossary as _glossary,
    lead_and_why as _lead_and_why,
    math as _math,
    plain as _plain,
    prose as _prose,
    render_page,
    split_lead as _split,
    static_figure as _static_figure,
    unit as _unit,
    why as _why,
)
...
def write_html(
    ...
    for path in static_figures or []:
        if Path(path).is_file():
            ...
            figures.append(_static_figure(Path(path), title, caption))
