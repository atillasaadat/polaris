...
| `magnetorquer.{hpp,cpp}` | Dipole limit, ±linearity, residual moment + hysteresis (play operator) | `commandDipole(m)`; torque `τ=m×B` applied by the environment path |

Implements **REQ-SIM-003** (actuator truth models) and **REQ-SIM-005** (fault
injection). CMGs and thrusters remain (§7); the allocation inverse `W⁺` is the
Phase-5 control layer (§8.5), not a truth model.
