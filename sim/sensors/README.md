...
| `gnss.{hpp,cpp}` | PVT fix: H/V-split position σ, velocity σ, clock bias; a **correlated (common-mode) position component** (Push 77) carried as a first-order Gauss-Markov process in the local geodetic frame — configured as a *fraction of the datasheet variance*, so the quoted total is preserved and only the error's spectrum changes, and the receiver reports the total either way; sample-rate gating, cold-start/reacquisition, **fix latency**; outage/spoof/clock-jump + geographic jamming | ECEF position/velocity @ **GPS time** |
