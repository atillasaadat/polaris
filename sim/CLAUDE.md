...
- **Accuracy is rarely one number.** Datasheets quote it against incidence angle, temperature, or field position because those are the axes it varies along (GomSpace FSS: 0.5° inside 45°, 2.0° to 60°). Carry the realised σ on the measurement so an estimator can be fed what the sensor actually had, not a headline.
- **One implementation per physical effect.** A force or torque belongs to exactly one `ForceTorqueModel`, because the plant sums them: the gravity-gradient couple lives in `world/gravity_gradient`, not inside `SphericalHarmonicGravity` (where it used to, and where it would be double-counted the moment the standalone provider was composed in), and it is the same rule the occlusion model above states for optical sensors. Before adding a torque, grep for it — the model that looks like the natural home may already return it.
