...
| `imu_voting.{hpp,cpp}` | Fault-tolerant combination of N redundant gyros (§8.2): per-unit plausibility gates (finiteness, vehicle rate limit), then a per-axis **median** at ≥ 3 units — never an average, whose breakdown point is zero, so one railed unit would drag the combined rate without limit [rousseeuw1987][gilmore1972]. Generic over N and carrying the whole ladder; the reference vehicle's **two** IMUs put it on the pairwise rung, where a disagreement is detected but attributable only against the MEKF's propagated rate, and reports no rate at all when that is unavailable. Exclusion is latched with a consecutive-plausible-cycle re-admission policy for the caller's FDIR (§9.2); REQ-ADET-008, REQ-ADET-009 |
