...
Two of those numbers are worth stating plainly, because they are the *measured* answer rather than the hoped-for one. **First, the coarse and fine thresholds are equal.** The systematic-floor argument above says a constant offset does not average down; the campaign quantifies it. With a 2.04° 1σ per-axis systematic on the sun pair and 1.93° on the magnetic pair, the coarse chain measures a median error norm of 3.3° and a 3σ bound of 8.9°, and the MEKF on the same two sources measures 2.9° and 11.4° — two campaigns compared distributionally rather than run-for-run, since they draw from different master seeds. The filter wins the bulk of the distribution and wins gyro-bias observability and a covariance that means something — but with only two vector sources, neither a fixed-gain blend nor an optimal filter can tell a constant offset from the truth, so both land on the same floor, and the tail is simply the Rayleigh tail of the systematic draw (3.44σ = 7.0° for the sun pair alone; the worst runs are the ones that drew a large sun *and* a large magnetic offset together, with no NIS rejections and no coasting involved). Improving this is a **sensor-budget** action — hard-iron calibration, albedo correction, a tracker — not an estimator-tuning action. **Second, the geometry condition is not a dodge.** The solution's component about the sun is fixed by the magnetic pair alone and its error grows as 1/sin(separation); over the full band the flight gate admits (down to sin 10°) the same campaign measures ~19° in both modes, driven entirely by runs below ~30° of separation. That is the observation geometry, not the estimator, and below the gate the designed behaviour is to refuse the solve and coast (REQ-ADET-002). **Third, on the norm metric a single star tracker is dominated by its about-boresight axis**: the AURIGA's Z terms RSS to ≈ 95 arcsec 3σ against ≈ 16 arcsec cross-boresight, plus the unit's fixed 61 arcsec bias (`bias_deg` 0.017°, a different term from the separately configured thermo-elastic drift) that does not average down at all, so ≈ 113 arcsec is the hardware alone — which is why REQ-ADET-007 is set at 0.05° rather than the ~0.02° a cross-boresight-only reading of the datasheet suggests. Two trackers with **non-parallel** boresights, each covering the other's weak axis, are what buy the 0.02°-class figure.
