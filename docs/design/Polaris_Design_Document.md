...
- **Star tracker into the MEKF:** the tracker joins the SS+MAG+IMU suite as a further vector/attitude source (IMU still propagates); this is what verifies REQ-ADET-007 and removes the white-`R` optimism of the sun/magnetometer-only budget. A **second tracker with a non-parallel boresight** covers the first's about-boresight weak axis (§8.1).
...
- **Phase 11 — Monte Carlo + analysis tools:** dispersion framework, momentum/sizing, detumble MC, contact scheduling, link budget, consistency metrics, per-REQ margin reporting; **RW jitter waterfall tool** (spin sweep → PSD-per-speed-bin spectrograms from the §7 harmonic model, keep-out-zone extraction feeding the §8.5 table).
