...
- **Phase 4 — Attitude determination:** initializers (TRIAD/QUEST), MEKF fine mode + **coarse mode (SS+MAG+IMU)**, multi-IMU/multi-sun-sensor fusion, validity flags + occlusion handling. *Started (Push 39):* the `lib/gnc` coarse chain — TRIAD with Shuster covariance + the coarse SS+MAG+IMU estimator behind the §10 Safe-mode floor (§8.1). Remaining: the F´ `AttitudeEstimator` component wiring it to the sensor and `OnboardTables` ports, MEKF fine mode, QUEST, and the fusion layer (§8.2).
