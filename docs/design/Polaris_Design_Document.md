...
The axis-resolved knowledge requirement this exists to make measurable is **REQ-PAY-001** (`docs/requirements/payload.rst`): the attitude-knowledge error projected onto the sensor's X–Y plane. See §8.1 for why the vehicle-level requirements stay on the total norm and the cross-boresight split belongs to the payload.
...
**The payload's share of that error, measured (Push 45).** The vehicle-level requirements stay on the total norm, and the axis-resolved form belongs to a payload — but "belongs to a payload" is only useful with a number attached, so the same campaign measures it. The metric is the **cross-boresight** error: the great-circle angle between where the payload's boresight actually points and where the attitude solution says it points, `∠(R(q̂)ᵀb, R(q)ᵀb)` for the mounted boresight `b` (sensor +Z, §6.3). It is the part of the knowledge error that displaces the scene on the focal plane; the remaining component is a rotation *about* the boresight, which turns the image about its own centre without moving where the instrument looks, and for a circularly symmetric instrument costs nothing at all. On the fine-mode runs the campaign measures a median of **2.2°** and a 3σ bound of **10.6°**, against 2.9° and 11.4° for the total norm on those same runs — the ratio of the medians, 0.77, is the expected one, since for an isotropically directed error the mean of sin ψ is π/4 = 0.785. So a payload pays about three quarters of the vehicle's knowledge error and the rest is roll it does not pay for. **REQ-PAY-001** takes 14° from that (24.5% margin), and the result is deliberately shown to be **independent of where the payload is mounted**: the knowledge error has no preferred body axis, and the campaign asserts it on a canted mounting rather than assuming it — a mounting-dependent answer would mean the campaign had a preferred axis, which would invalidate the vehicle-level numbers too. This is a **knowledge** bound, not a pointing one; control error adds in quadrature (§8.4), and boresight *stability* over an integration time is a separate quantity the §7 micro-vibration work owns.
