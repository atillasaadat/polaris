...
- **Phase 4 — Attitude determination:** initializers (TRIAD/QUEST), MEKF fine mode + **coarse mode (SS+MAG+IMU)**, multi-IMU/multi-sun-sensor fusion, validity flags + occlusion handling. *Started (Push 39, 40):* the `lib/gnc` coarse chain — TRIAD with Shuster covariance + the coarse SS+MAG+IMU estimator behind the §10 Safe-mode floor — and the F´ `AttitudeEstimator` component running it on the barrier-driven 10 Hz GNC cycle, fed by the `GncPorts` measurement seam (port arrays, multi-unit ready) and the `OnboardTables`/IGRF-14 references (§8.1). Remaining: the config-compiler→`ParameterDb` tuning path the estimator refuses without, an onboard position source for the magnetic reference that does not depend on a live GNSS fix (§8.3), MEKF fine mode with its NEES/NIS diagnostics, QUEST, and the multi-unit fusion layer (§8.2).
