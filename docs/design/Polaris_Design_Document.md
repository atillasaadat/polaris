...
| **Third-body / multi-body** | Point-mass from JPL ephemeris | **SPICE / NAIF (DE440/DE441)** on ground/truth. Sun + Moon by default; **planetary perturbers (Mercury→Neptune, DE440 barycenters + system GMs) are config-selectable** via `third_bodies` — Jupiter/Venus are the largest at ~1e-7 of the lunar term in LEO, carried for completeness studies. The committed Chebyshev fixture (`tests/golden/de440_bodies.cheb`) holds all nine bodies; a planet requested but absent from the fixture refuses to build rather than silently skipping. |
