...
TEST(SitlAttitudeTuning, CompiledParametersLetTheEstimatorAcquireAttitude) {
  ...
  EXPECT_NE(log.find("PrmFileLoadComplete"), std::string::npos)
      ...
      << log;
  EXPECT_NE(log.find("Records: 153"), std::string::npos)
      << "prmDb loaded a record count other than the 153 declared parameters "
         ...
         "correlated-GNSS R-inflation parameters GnssCorrSigmaHM/VM):\n"
      << log;
