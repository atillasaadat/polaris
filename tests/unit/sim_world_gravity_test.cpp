/// @file Tests for spherical-harmonic gravity (REQ-SIM-002).
///
...
///  - and the model must be torque-free: the gravity-gradient couple is its own
///    §5.3 provider (sim_world_gravity_gradient_test.cpp), and returning it here
///    as well would double-count it in the composite.
...
TEST(SphericalHarmonicGravity, DegreeZeroReducesToPointMass) {
  ...
  const world::SphericalHarmonicGravity g(world::GravityCoeffs::earthZonal(), 0, 0);
...
TEST(SphericalHarmonicGravity, J2TermMatchesClosedForm) {
  RecordProperty("verifies", "REQ-SIM-002");
  const world::SphericalHarmonicGravity g(world::GravityCoeffs::earthZonal(), 2, 0);
...
TEST(SphericalHarmonicGravity, AccelerationIsGradientOfPotential) {
  ...
  const world::SphericalHarmonicGravity g(c, 2, 2);
...
TEST(SphericalHarmonicGravity, J2CausesAnalyticNodalRegression) {
  ...
  const world::SphericalHarmonicGravity gravity(world::GravityCoeffs::earthZonal(), 2, 0);
...
TEST(SphericalHarmonicGravity, IsTorqueFree) {
  RecordProperty("verifies", "REQ-SIM-002");
  // This model used to return the gravity-gradient torque as well. It no longer
  // does: the couple is its own §5.3 provider (world/gravity_gradient.hpp), and a
  // torque here would be summed twice by the composite whenever both are wired.
  // See sim_world_gravity_gradient_test.cpp for the torque's own coverage.
  const Eigen::Matrix3d j = Eigen::Vector3d(10.0, 20.0, 30.0).asDiagonal();
  const world::SphericalHarmonicGravity g(world::GravityCoeffs::earthZonal(), 4, 0);

  ps::TruthState s;
  s.position = pm::Vec3<pf::ECI>(Eigen::Vector3d(-7.0e6 * Eigen::Vector3d(1, 1, 1).normalized()));
  s.attitude = pm::Quat<pf::Body, pf::ECI>(
      pm::Quaternion::FromAxisAngle(Eigen::Vector3d::UnitZ(), M_PI / 6.0));
  EXPECT_EQ(g.torque(s).eigen(), Eigen::Vector3d::Zero());
  // ...while the standalone provider, on the same state and that inertia, does
  // produce one — so this is a real property of the gravity model, not a state
  // where the torque happens to vanish.
  EXPECT_GT(world::GravityGradientTorque(j, kMu).torque(s).eigen().norm(), 0.0);
}
...
TEST(SphericalHarmonicGravity, DegreeAndOrderClampToTable) {
  RecordProperty("verifies", "REQ-SIM-002");
  const world::SphericalHarmonicGravity g(world::GravityCoeffs::earthZonal(), 99, 99);
...
TEST(SphericalHarmonicGravity, NormalizedZonalMatchesUnnormalizedOracle) {
  ...
  const world::SphericalHarmonicGravity g(world::GravityCoeffs::earthZonal(), 6, 0);
...
TEST(SphericalHarmonicGravity, AccelerationIsGradientAt200x200) {
  ...
  const world::SphericalHarmonicGravity g(c, kN, kN);
...
TEST(SphericalHarmonicGravity, AccelerationIsGradientNonSquare200x5) {
  ...
  const world::SphericalHarmonicGravity g(c, kN, kM);
  EXPECT_EQ(g.degree(), kN);
...
TEST(SphericalHarmonicGravity, FiniteAtPoleAndHighDegree) {
  ...
  const world::SphericalHarmonicGravity gz(world::GravityCoeffs::earthZonal(), 6, 0);
